...
async function runTest() {
  ...
  try {
    ...
    // Test the new BPMNER rule (duplicate diagrams)
...
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" 
  ...
  <bpmndi:BPMNDiagram id="Diagram_2"><bpmndi:BPMNPlane id="Plane_2" bpmnElement="Process_1" /></bpmndi:BPMNDiagram>
    ...
    const hasDupDiagramIssue = dupIssues.some(i => i.rule === 'bpmner/gen-02-no-duplicate-diagrams');
    
    if (hasDupDiagramIssue) {
      console.log('SUCCESS: Bundle correctly identified BPMNER gen-02 duplicate diagram issue.');
    } else {
      console.error('FAILED: Did not find BPMNER gen-02 issue in bad XML. Issues found:', JSON.stringify(dupIssues, null, 2));
      process.exit(1);
    }
