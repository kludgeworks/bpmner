...
class BpmnLintServiceIntegrationTest {
    ...
    fun `legacy duplicate diagram config and docs resolve through canonical rule`() {
        ...
        assertTrue(issues.any { it.rule == "bpmner/gen-no-duplicate-diagrams" }, "Legacy config should report canonical rule")
        assertTrue(issues.none { it.rule == "bpmner/gen-02-no-duplicate-diagrams" }, "Legacy rule id should not leak into diagnostics")
    ...
    fun `init fails fast for unknown lint rule id`() {
        ...
        assertTrue(exception.message!!.contains("bpmner/act-verb-object-name"), "Error message should contain the unknown rule id")
    }
}
