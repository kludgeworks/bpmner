...
class BpmnGeneratorAgentTest {
    ...
    fun `auto-fix no-op keeps original validated xml`() {
        ...
        val agent = buildAgent(BpmnConfig(maxAttempts = 3), lintService, RecordingXsdValidator(listOf(emptyList())), RecordingConverter())
    ...
    fun `auto-fix unavailable keeps original validated xml for layout`() {
        ...
        val agent = buildAgent(
            ...
            lintService,
            RecordingXsdValidator(listOf(emptyList())),
            RecordingConverter(),
            layoutService,
        )
    ...
    fun `auto-fix that invalidates XSD keeps original validated xml for layout`() {
        ...
        val layoutService = RecordingLayoutService()
        val agent = buildAgent(BpmnConfig(maxAttempts = 3), lintService, xsdValidator, RecordingConverter(), layoutService)
