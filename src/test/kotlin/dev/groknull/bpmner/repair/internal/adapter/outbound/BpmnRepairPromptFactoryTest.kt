...
class BpmnRepairPromptFactoryTest {
    ...
    fun `patchFeedback annotates failed-local diagnostics and includes LLM diagnostics`() {
        ...
        val llmDiag = lintDiagnostic("bpmner/name-02", "Task_1", "Use action verb", BpmnRepairRoute.LLM)
        val localFailedDiag = lintDiagnostic("bpmner/name-01", "Task_2", "Strip type word", BpmnRepairRoute.LOCAL_XML)
        val outcome =
    ...
    fun `patchFeedback without local outcome renders diagnostics without local-fix marker`() {
        val factory = factory()
        val diag = lintDiagnostic("bpmner/name-02", "Task_1", "Use action verb", BpmnRepairRoute.LLM)
