...
internal open class BpmnDefinitionToXmlConverter : BpmnRenderer {
    ...
    override fun render(definition: BpmnDefinition): RenderedBpmn {
        ...
        val xml = addCatalogsAndEventDefinitions(output.toString(Charsets.UTF_8), listOf(definition))
        return RenderedBpmn(
    ...
    /**
     * Renders a multi-participant [BpmnCollaboration] to BPMN XML: a `<collaboration>` root with
     * one `<participant>` per pool, one `<process>` per pool, and the cross-pool `<messageFlow>`s.
     * Returns the XML string directly — the pipeline carriers stay [BpmnDefinition]-typed; wiring
     * the collaboration through generation/repair/layout is handled separately.
     */
    fun toXml(collaboration: BpmnCollaboration): String {
        val modelInstance = toModelInstance(collaboration)
        val output = ByteArrayOutputStream()
        Bpmn.writeModelToStream(output, modelInstance)
        val processes = collaboration.participants.map { it.process }
        val xml = addCatalogsAndEventDefinitions(output.toString(Charsets.UTF_8), processes)
        return stripUnusedDiNamespaces(xml)
    }
    ...
    private fun toModelInstance(definition: BpmnDefinition): BpmnModelInstance {
        ...
        populateProcess(modelInstance, definition, process)
        removeAutoGeneratedDiagramInterchange(modelInstance, definitions)
        return modelInstance
    }

    private fun toModelInstance(collaboration: BpmnCollaboration): BpmnModelInstance {
        ...
        val collaborationElement = modelInstance.newInstance(Collaboration::class.java)
        collaborationElement.id = collaboration.id
        definitions.addChildElement(collaborationElement)

        // Build every participant's process first so message-flow source/target can resolve to
        // FlowNode instances across pools.
        val nodesById = mutableMapOf<String, FlowNode>()
        for (pool in collaboration.participants) {
            val process = modelInstance.newInstance(Process::class.java)
            process.id = pool.process.processId
            process.name = pool.process.processName
            definitions.addChildElement(process)
            nodesById += populateProcess(modelInstance, pool.process, process)

            val participant = modelInstance.newInstance(Participant::class.java)
            participant.id = pool.id
            participant.name = pool.name
            participant.process = process
            collaborationElement.participants.add(participant)
        }

        for (messageFlow in collaboration.messageFlows) {
            val element = modelInstance.newInstance(MessageFlow::class.java)
            element.id = messageFlow.id
            element.source = messageFlowEndpoint(nodesById, messageFlow.sourceRef, messageFlow.id, "sourceRef")
            element.target = messageFlowEndpoint(nodesById, messageFlow.targetRef, messageFlow.id, "targetRef")
            collaborationElement.messageFlows.add(element)
        }
    ...
    // Message-flow endpoints must be BPMN InteractionNodes (tasks/events/etc.), not arbitrary
    // flow nodes — gateways can't send or receive messages. Resolve by id and enforce the cast.
    private fun messageFlowEndpoint(
        ...
        val node =
            nodesById[ref]
                ?: error("Unknown message-flow $role '$ref' on '$messageFlowId'")
        return node as? InteractionNode
            ?: error("Message-flow $role '$ref' on '$messageFlowId' is a ${node::class.simpleName}, not a valid endpoint")
    }

    private fun populateProcess(
        ...
        buildLaneSet(modelInstance, definition, process, nodeMap)
        return nodeMap
    }
    ...
    // Lanes are a swimlane partition inside the process. The BPMN XSD requires <laneSet> to
    // precede the flow elements in tProcess; the Camunda child-element collection inserts it at
    // the schema-correct position, so we build it after the nodes/flows exist (the lane's
    // flowNodeRef children must point at already-created FlowNode instances).
    private fun buildLaneSet(
        modelInstance: BpmnModelInstance,
        definition: BpmnDefinition,
        process: Process,
        nodeMap: Map<String, FlowNode>,
        ...
        val laneSet = modelInstance.newInstance(LaneSet::class.java)
        laneSet.id = "LaneSet_${definition.processId}"
        for (lane in definition.lanes) {
            val laneElement = modelInstance.newInstance(Lane::class.java)
            laneElement.id = lane.id
            if (lane.name.isNotBlank()) laneElement.name = lane.name
            for (ref in lane.flowNodeRefs) {
                val flowNode =
                    nodeMap[ref]
                        ?: throw IllegalArgumentException("Unknown flowNodeRef '$ref' on lane '${lane.id}'")
                laneElement.flowNodeRefs.add(flowNode)
            }
            laneSet.lanes.add(laneElement)
        }
    ...
    private fun addCatalogsAndEventDefinitions(
        xml: String,
        processes: List<BpmnDefinition>,
    ): String {
        val document = parseDocument(xml)
        val root = document.documentElement
        // Catalogs are definitions-level rootElements; in a collaboration they go before the
        // first <process>. Per-node/per-task stamping below resolves elements by id across the
        // whole document, so it works unchanged for any number of participant processes.
        ...
        processes.flatMap { it.escalations }.asReversed().forEach { escalation ->
            root.insertBefore(
        ...
        processes.flatMap { it.errors }.asReversed().forEach { error ->
            root.insertBefore(
        ...
        processes.flatMap { it.signals }.asReversed().forEach { signal ->
            root.insertBefore(
        ...
        processes.flatMap { it.messages }.asReversed().forEach { message ->
            root.insertBefore(
        ...
        processes.flatMap { it.nodes }.forEach { node ->
            when (node) {
                is BpmnStartEvent -> {
