...
/**
 * Annotation-free BPMN language + SPI. Every concrete BPMN data class in `bpmn/internal/model/BpmnDomain.kt`
 * implements one of these interfaces. Rule engines (compiled and Pkl-authored) consume the bpmn
 * types so they never depend on Jackson, Jakarta Validation, or any framework-side detail of the
 * domain types.
 *
 * The hierarchy is sealed all the way down, which gives bpmn-side `when` blocks compile-time
 * exhaustiveness: every new BPMN subtype must be added to both root and internal/model in a single
 * change. The cost is that bpmn/ must stay in one Kotlin compilation module — if the root tier
...
/**
 * A BPMN node. Implementations live in `bpmn/internal/model/BpmnDomain.kt`; each carries the Jackson
...
/**
 * Multi-instance loop characteristics attached to a [BpmnTask]. Annotation-free bpmn view;
 * the concrete data class (with Jackson / Jakarta annotations) lives in `bpmn/internal/model/BpmnDomain.kt`
...
/**
 * Standard-loop characteristics attached to a [BpmnTask]. Annotation-free bpmn view; the concrete
 * data class lives in `bpmn/internal/model/BpmnDomain.kt` and renders to `<bpmn:standardLoopCharacteristics>`
 * on the task element.
 */
...
// classes (in `dev.groknull.bpmner.bpmn.internal.model`) from implementing them. Exhaustive `when` over
 ...
 * non-sealed so concrete `bpmn.internal.model` data classes (in a different package) can implement it
 ...
 * concrete `bpmn.internal.model` data classes. Kept as a property extension to preserve the existing
 ...
 * `@JsonSubTypes` annotation on `bpmn.internal.model.BpmnNode`. The exhaustive `when` catches missing
