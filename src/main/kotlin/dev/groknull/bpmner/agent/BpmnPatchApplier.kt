...
class BpmnPatchApplier {
    ...
    private fun applySetEdgeLabel(definition: BpmnDefinition, op: BpmnPatchOperation): OperationResult {
        ...
        if (edge.name == label) return OperationResult.Unchanged
        val updated = definition.copy(sequences = definition.sequences.map { if (it.id == edgeId) it.copy(name = label) else it })
        return OperationResult.Changed(updated)
    }
