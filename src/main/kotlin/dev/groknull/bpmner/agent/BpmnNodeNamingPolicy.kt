...
object BpmnNodeNamingPolicy {
    ...
    fun requiresName(node: BpmnNode, incomingCount: Int, outgoingCount: Int): Boolean =
        when (node.type) {
            NodeType.START_EVENT,
            NodeType.USER_TASK,
            NodeType.SERVICE_TASK,
            NodeType.END_EVENT -> true
            NodeType.EXCLUSIVE_GATEWAY -> outgoingCount > 1
        }

    fun missingNameMessage(node: BpmnNode): String =
        "node ${node.id.ifBlank { "<blank>" }} name must not be blank for ${node.type}"
}
