...
@Component
class BpmnRefinementEngine(
    private val config: BpmnConfig,
    private val bpmnConverter: BpmnDefinitionToXmlConverter,
    private val evaluationPipeline: BpmnEvaluationPipeline,
    ...
    fun refine(
        ...
        var currentAttempt = BpmnRepairAttempt(
            ...
            repairAttempts = 0,
            graph = currentGraph,
            evaluation = evaluationPipeline.evaluate(
                graph = currentGraph,
        ...
        var currentRecord = evaluationPipeline.toRecord(currentAttempt)
        ...
        if (currentAttempt.evaluation.isSuccessful()) {
            ...
            return currentAttempt.evaluation.toValidatedBpmnXml(currentAttempt.repairAttempts)
        }

        while (history.size < maxEvaluations) {
            evaluationPipeline.logDiagnosticSummary(currentAttempt.evaluation.globalDiagnostics.diagnostics)
            ...
            val repaired = when (repair) {
                ...
                is BpmnRepairResult.TerminalFailure -> failRefinement(maxEvaluations, history, repair.reason)
            ...
            val nextEvaluation = evaluationPipeline.evaluate(
                graph = currentGraph,
            ...
            val nextRecord = evaluationPipeline.toRecord(
                attempt = nextAttempt,
            ...
            if (nextAttempt.evaluation.isSuccessful()) {
                ...
                return nextAttempt.evaluation.toValidatedBpmnXml(nextAttempt.repairAttempts)
            }
