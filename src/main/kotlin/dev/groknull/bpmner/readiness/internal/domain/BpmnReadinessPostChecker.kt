...
internal class BpmnReadinessPostChecker(
    ...
    private fun verdictFor(overallScore: Int): ReadinessVerdict = if (overallScore >= config.readyThreshold) {
    ...
    private fun guidingWorkflowQuestion(): ClarificationQuestion = ClarificationQuestion(
        id = "q1",
    ...
    private fun questionTextFor(area: MissingProcessArea): String = when (area) {
    ...
    private fun dimensionFor(area: MissingProcessArea): ReadinessDimension? = when (area) {
