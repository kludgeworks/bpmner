...
internal class BpmnReadinessPromptFactory(
    ...
    fun prompt(request: BpmnRequest): String =
        buildString {
            appendHeader()
            appendWorkflowDefinition()
            appendEvaluationRules()
            appendVerdictRules(config)
            appendDimensionGuidance()
            appendMissingAreas()
            appendClarificationRules(config)
            appendOriginalRequest(request)
        }
    ...
    private fun StringBuilder.appendWorkflowDefinition() {
    ...
    private fun StringBuilder.appendEvaluationRules() {
    ...
    private fun StringBuilder.appendVerdictRules(config: BpmnReadinessConfig) {
    ...
    private fun StringBuilder.appendDimensionGuidance() {
    ...
    private fun StringBuilder.appendMissingAreas() {
        appendLine("Use missing process areas only from this vocabulary:")
        MissingProcessArea.entries.forEach { appendLine("- ${it.name}") }
        appendLine()
    }

    private fun StringBuilder.appendClarificationRules(config: BpmnReadinessConfig) {
    ...
    private fun StringBuilder.appendOriginalRequest(request: BpmnRequest) {
