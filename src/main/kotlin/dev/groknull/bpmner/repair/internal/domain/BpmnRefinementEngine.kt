...
internal class BpmnRefinementEngine(
    ...
    fun refine(
        ...
        var state = createInitialState(request, graph, rendered)

        if (state.attempt.evaluation.isSuccessful()) {
            return publishSuccessAndReturn(context, state.attempt, request)
        }
    ...
    private fun publishSuccessAndReturn(
        ...
        context.updateProgress("Validation passed after ${attempt.repairAttempts} repair attempt(s)")
        val result = attempt.evaluation.toValidatedBpmnXml(attempt.repairAttempts)
        eventPublisher.publishEvent(BpmnValidationPassedEvent(request, result.xml, attempt.repairAttempts))
        return result
    }
