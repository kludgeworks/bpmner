...
/** The shared fail-fast seam every structured-output role call runs through: catches Embabel's
 *  two stable InvalidLlmReturn* exceptions, publishes [StructuredOutputFailureEvent] for
 *  observability, then rethrows so each role keeps its own exception translation on top. Any
 *  other exception (network, rate limit) propagates untouched — only these two get this
 *  treatment, which is the transient-vs-deterministic classification itself. */
