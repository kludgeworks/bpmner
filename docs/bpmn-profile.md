...
| `BoundaryShapePlacement` | Boundary-event shapes are projected from the boundary's own exception edge start point (AD-622-08): the exception edge is a real ELK edge sourced at the boundary's SOUTH port, so ELK's own port distribution and crossing minimisation — not an independent placement algorithm — separate multiple attachments (`BoundaryShapePlacement.kt:14-20`). | `BpmnToElkMapperTest.kt`, `BpmnPlacementPassTest.kt` + golden corpus | structurally exempt (boundary shapes outside lane bands) |
...
| 4 | Boundary-event attachment via ELK port geometry; shapes projected from that geometry | `BpmnToElkMapperTest.kt` (AD-622-08 port→handler edge), placement exception ledger above |
