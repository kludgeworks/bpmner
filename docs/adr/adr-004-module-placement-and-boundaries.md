...
- **Capability root** — `@ConfigurationProperties`, `@DomainEvent`, and public
  `@ApplicationRing` capability APIs for that capability only.
- **`<cap>/internal/domain/`** — domain services (`@Service`); no Spring stereotypes beyond
  `@Component`/`@Service`, no IO, no cross-module imports.
- **`<cap>/internal/adapter/inbound/`** — `@InfrastructureRing` classes (CLI, REST, agent actions).
- **`<cap>/internal/adapter/outbound/`** — `@InfrastructureRing` classes (integrations).
