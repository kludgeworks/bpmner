...
Override per-role under `embabel.models.llms.<role>`. Provider profiles are configured in `application-anthropic.yaml`, `application-openai.yaml`, `application-gemini.yaml`, `application-mistral.yaml`, `application-deepseek.yaml`, `application-llama.yaml`, and `application-githubmodels.yaml`. The simplest way to run is `mise run bpmner-cli --provider <provider>` — one of `anthropic`, `openai`, `gemini`, `mistral`, `deepseek`, `llama`, or `githubmodels`. For most providers, the task reads the API key from 1Password (`op://bpmner/<provider>/api-key`); for `githubmodels`, it retrieves the token via the GitHub CLI (`gh auth token`). In either case, it sets `SPRING_PROFILES_ACTIVE` for you (add `--web` or `--verbose` to layer on those profiles). To run `bazelisk` directly instead, pass `--spring.profiles.active=<provider>` (or set `SPRING_PROFILES_ACTIVE`) and ensure the corresponding API key environment variable is set (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `MISTRAL_API_KEY`, `DEEPSEEK_API_KEY`, `OPENROUTER_API_KEY`, or `GITHUB_TOKEN`).
...
Meaning: `Budget(actions = N)` exhausted before the goal was reached. `ReplanRequestedException` does not count against this budget — only successful action completions do. For the lifecycle context, see [architecture §4](./architecture.md#4-goap-lifecycle-and-repair-architecture).
