...
| `layout` | `ValidatedBpmnXml → FinalValidatedBpmnXml` | (inline) | `BpmnLayoutPort` + `BpmnXsdValidationPort` + `BpmnXmlParser` directly; errors on XSD-invalid output, and on a laid-out XML that no longer matches the `BpmnDefinition` carried alongside it (parsed back and compared by `semanticDivergenceFrom`, which ignores diagram interchange and document order). A result that cannot be parsed at all is logged and allowed through: layout is the last stage that can still be judged against the definition, so an unverifiable result is not the same as a wrong one. |
