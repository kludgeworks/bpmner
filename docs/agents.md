...
Seventeen `@Action` methods total. The `BpmnProgressProjectionObserver` maps every one to a user-facing progress label — see [`operator-guide.md`](./operator-guide.md#progress-events-sse).
