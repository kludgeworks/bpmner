...
describe('BpmnLayoutApi bundle smoke test', () => {
  ...
  before(() => {
    ...
    const globalApi = (globalThis as typeof globalThis & { BpmnLayoutApi?: BpmnLayoutApi }).BpmnLayoutApi;
  ...
  it('returns layouted XML for a minimal process', async () => {
    ...
    assert.ok(result.includes('<definitions'), 'result should contain BPMN definitions');
  });
