...
4. **Diagnostic stamping.** When the lint port returns `LintIssue`s, `BpmnDiagnosticNormalizer.normalizeLintDiagnostics()` strips the `klm/` or `bpmnlint-plugin-klm/` prefix, looks up the capability, and stamps each `BpmnDiagnostic` with `kind`, `repairSafety`, and `fixHandler`. Diagnostics from XSD or graph sources are not lint rules and carry `kind = null`.
