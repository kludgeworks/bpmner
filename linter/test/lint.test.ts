...
describe('lint rules', () => {
  ...
  it('every manifest rule has generated markdown docs', () => {
    for (const rule of tsRules) {
      assert.equal(typeof customRuleDocs[`${KLM_PLUGIN_PREFIX}/${rule.id}`], 'string');
  ...
  it('migrated rule metadata maps Pkl severity to bpmnlint levels', () => {
    ...
    assert.equal(getBpmnlintLevel('bpmnlint-plugin-klm/act-discouraged-business-verbs'), 'warn');
  });
  ...
  it('gen-klops-allowed-elements', async () => {
    assert.ok(hasRule(await lint(fixtures.gen01Choreography), 'gen-klops-allowed-elements'));
  });
