...
describe("lint rules", () => {
	...
	it("every manifest rule has generated markdown docs", () => {
		for (const rule of tsRules) {
			assert.equal(
				typeof customRuleDocs[`${KLM_PLUGIN_PREFIX}/${rule.id}`],
				"string",
			)
			assert.ok(customRuleDocs[`${KLM_PLUGIN_PREFIX}/${rule.id}`].length > 0)
		}
	})
	...
	it("legacy duplicate diagram metadata resolves to the canonical rule", () => {
		const config = getRuleConfig("klm/gen-02-no-duplicate-diagrams")

		assert.equal(config.id, "gen-no-duplicate-diagrams")
		assert.ok(resolver.resolveRule("", "gen-02-no-duplicate-diagrams"))
		assert.ok(resolver.resolveRule("klm", "gen-02-no-duplicate-diagrams"))
		assert.ok(
	...
	it("migrated rule metadata maps Pkl severity to bpmnlint levels", () => {
		assert.equal(getBpmnlintLevel("klm/name-uncommon-abbreviations"), "warn")
		assert.equal(
			getBpmnlintLevel("bpmnlint-plugin-klm/act-discouraged-business-verbs"),
			"warn",
		)
	})
	...
	it("gen-klops-allowed-elements", async () => {
		assert.ok(
			hasRule(
				await lint(fixtures.gen01Choreography),
				"gen-klops-allowed-elements",
			),
		)
	})
