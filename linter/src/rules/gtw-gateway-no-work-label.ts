...
const WORK_VERB_STARTERS = new Set([
  'check', 'validate', 'verify', 'approve', 'reject', 'review', 'assess', 'confirm'
]);
...
function startsWithActionVerb(name: string): boolean {
  ...
  const firstWord = name.split(/\s+/)[0]?.toLowerCase().replace(/^[^a-z]+|[^a-z]+$/g, '') || '';
...
export = function() {
  function check(node: ModdleElement, reporter: Reporter) {
    ...
    if (startsWithActionVerb(name)) {
      reporter.report(node.id, 'Gateway label should describe a decision condition, not perform work');
    }
  }
