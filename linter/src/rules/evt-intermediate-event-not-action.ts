...
const TARGET_TYPES = [ 'bpmn:IntermediateCatchEvent', 'bpmn:IntermediateThrowEvent' ];
...
export = function() {
  function check(node: ModdleElement, reporter: Reporter) {
    ...
    if (startsWithVerbLike(name)) {
      reporter.report(node.id, 'Intermediate event name should describe a state, not an action');
    }
  }
