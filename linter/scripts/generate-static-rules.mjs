	...
	.map((r) => {
		...
		return [
			...
			...(r.aliases ?? []).flatMap((alias) => [
				...
				`  'klm/${alias}': ${JSON.stringify(aliasContent)},`,
				`  'bpmnlint-plugin-klm/${alias}': ${JSON.stringify(aliasContent)},`,
			]),
		].join("\n")
	})
	.filter(Boolean)
...
const customRuleAliasEntries = tsRules
	.flatMap((r) =>
		(r.aliases ?? []).flatMap((alias) => [
			`  '${alias}': '${r.id}',`,
			`  'klm/${alias}': 'klm/${r.id}',`,
			`  'bpmnlint-plugin-klm/${alias}': 'bpmnlint-plugin-klm/${r.id}',`,
		]),
	)
	.join("\n")
