...
The release also publishes a minimal Java OCI image index to `ghcr.io/kludgeworks/bpmner` with the immutable release version and `latest` tags. Both `linux/amd64` and `linux/arm64` are supported; Docker selects the matching platform automatically.
