...
- A custom **Pkl-authored rule catalog** with specialized rules enforcing industry best practices (naming conventions and structural logic). Rules are evaluated in-process by a Kotlin `RuleEngine`; diagram auto-layout is a JVM-native ELK pass. See [`docs/bpmn-profile.md`](docs/bpmn-profile.md) for the retained BPMN vocabulary and topology policy.
